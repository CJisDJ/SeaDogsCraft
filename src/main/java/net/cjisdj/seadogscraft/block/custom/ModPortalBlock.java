...
import net.cjisdj.seadogscraft.worldgen.portal.ModTeleporter;
...
public class ModPortalBlock extends Block {
    ...
    private void handleKaupenPortal(Entity player, BlockPos pPos) {
        if (player.level() instanceof ServerLevel serverlevel) {
            ...
            ResourceKey<Level> resourcekey = player.level().dimension() == ModDimensions.AFRICA_LEVEL_KEY ?
                    Level.OVERWORLD : ModDimensions.AFRICA_LEVEL_KEY;

            ServerLevel portalDimension = minecraftserver.getLevel(resourcekey);
            if (portalDimension != null && !player.isPassenger()) {
                if(resourcekey == ModDimensions.AFRICA_LEVEL_KEY) {
