...
public class CannonProjectileEntity extends Projectile {
    private static final EntityDataAccessor<Boolean> HIT =
            SynchedEntityData.defineId(CannonProjectileEntity.class, EntityDataSerializers.BOOLEAN);
    private int counter = 0;

    private boolean explodeTimer = false;
    private boolean hasHit = false;

    public CannonProjectileEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public CannonProjectileEntity(Level pLevel, Player player) {
        super(ModEntities.CANNON_PROJECTILE.get(), pLevel);
        setOwner(player);
...
//        }
        if (explodeTimer) {
            this.level().explode(null,this.getX(),this.getY(),this.getZ(),2f, Level.ExplosionInteraction.TNT);
        }
