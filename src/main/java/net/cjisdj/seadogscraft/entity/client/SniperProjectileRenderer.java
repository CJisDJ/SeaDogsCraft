...
public class SniperProjectileRenderer extends EntityRenderer<SniperProjectileEntity> {
    ...
    public SniperProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new SniperProjectileModel(pContext.bakeLayer(ModModelLayers.SNIPER_PROJECTILE_LAYER));
        this.shadowRadius = 0.5f;
    }

    public void render(SniperProjectileEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(SniperProjectileEntity pEntity) {
        return TEXTURE;
    }
}