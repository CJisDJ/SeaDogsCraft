...
public class CommonProjectileRenderer extends EntityRenderer<CommonProjectileEntity> {
    ...
    public CommonProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new SniperProjectileModel(pContext.bakeLayer(ModModelLayers.SNIPER_PROJECTILE_LAYER));
        this.shadowRadius = 0.5f;
    }

    public void render(CommonProjectileEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(CommonProjectileEntity pEntity) {
        return TEXTURE;
    }
}